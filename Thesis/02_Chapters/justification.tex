...
In environments where neither incremental nor continual learning is employed, each data 
incorporation requires retraining the entire model with all the available information. 
While recent continual learning approaches have partially addressed this by retraining 
only on a selected subset of the most relevant past data \cite{rebuffi2017icarl, 
krawczyk2017ensemble}, this still implies storing and managing historical data, which 
raises concerns regarding memory consumption, scalability, and data privacy in 
resource-constrained or sensitive deployment scenarios \cite{de2021continual}.  
